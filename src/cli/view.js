...
export function renderStatus({origin,startedAt,color,width=80,message=''}){
  const minutes=Math.floor((Date.now()-startedAt)/60000),duration=minutes<60?`${minutes}m`:`${Math.floor(minutes/60)}h ${minutes%60}m`;
  const lines=['  ◆ Vault unlocked','',`  Local    ${origin}`,`  Session  ${duration}`, '',width<48?'  O open · L link':'  O open browser · L show launch link','  Ctrl+C lock & quit'];
