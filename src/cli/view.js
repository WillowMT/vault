...
export function renderStatus({origin,startedAt,color,width=80,message='',locked=false,enrolling=false}){
  const minutes=Math.floor((Date.now()-startedAt)/60000),duration=minutes<60?`${minutes}m`:`${Math.floor(minutes/60)}h ${minutes%60}m`;
  const state=locked?'locked - waiting for passkey or recovery':enrolling?'unlocked - passkey setup required':'unlocked';
  const controls=locked?(width<48?'  O open · L link · R recover':'  O open browser · L show link · R recovery password'):enrolling?(width<48?'  O open · L setup link':'  O open browser · L show setup link'):(width<48?'  O open · L link':'  O open browser · L show launch link');
  const lines=[`  ◆ Vault ${state}`,'',`  Local    ${origin}`,`  Session  ${duration}`, '',controls,'  Ctrl+C lock & quit'];
