...
export function createSession(){
  ...
  function authorize(){if(closed)throw new Error('Server closed');launch='';session=token();csrf=token();return {cookie:`secretcli=${session}; HttpOnly; SameSite=Strict; Path=/`,csrfToken:csrf};}
  return {
    renew(){if(closed)throw new Error('Server closed');launch=token();expires=Date.now()+60000;return launch;},
    exchange(value){if(closed||!launch||Date.now()>expires||!equal(value,launch))throw new VaultError('Launch link expired. Press O in the CLI to open a new one.',401);return authorize();},
    authorize,
